...
function adicionarArestas(cy,noAdicionado,configuracoesDeCriacao){
    ...
    cy.nodes().forEach(element => {
      if(element.id()!=noAdicionado.id()){
       var arestaAdicionada2 =  cy.add({ data: {label:'2', source:noAdicionado.id() , target: element.id(), directed :true,color :'blue'}})
...
function adicionarListenersNos(noAdicionado,evt,configuracoesDeCriacao,cy){
    noAdicionado.on("tapdrag", evt=>{
        configuracoesDeCriacao.estaEmUmNo=true;
    })
    noAdicionado.on("cxttapstart",evt=>
    {
        configuracoesDeCriacao.passivelDeSerFeitoAresta = true;
    
    
        switch(configuracoesDeCriacao.paridadeNoAnterior){

            case 0:
            configuracoesDeCriacao.noAnterior = noAdicionado;
            configuracoesDeCriacao.paridadeNoAnterior++;
            break;
            
            case 1:
                //desenhar aresta
            if(configuracoesDeCriacao.noAnterior.id()== noAdicionado.id())
                return;
            
            if(parseInt(configuracoesDeCriacao.noAnterior.edgesTo(noAdicionado).size())>0){
                configuracoesDeCriacao.paridadeNoAnterior=0;
                return;
            }
               var aresta = cy.add({ data: {label:'1', source:configuracoesDeCriacao.noAnterior.id() , target: noAdicionado.id(), directed :true
            }});


                aresta.style({'curve-style': 'bezier',
                ...
                aresta.on("mousedown", evt=>{
                    
                document.getElementById("noInicio").value =cy.getElementById(aresta.data("source")).data("label");
                document.getElementById("noFim").value =cy.getElementById(aresta.data("target")).data("label");
                var inputPeso = document.getElementById("peso");
                inputPeso.value = parseFloat(aresta.data("label"));
                ...
                aresta.on("mouseover", evt=>{
                    configuracoesDeCriacao.estaEmUmNo=true;
                })
            
                aresta.on("mouseout", evt=>{
                    configuracoesDeCriacao.estaEmUmNo=false;
                })
    ...
    noAdicionado.on("click", evt=>{
       ...
       if($("#InicioDjikstra").lenght>0)
        document.getElementById("InicioDjikstra").value = valorDoLabel;
...
function desenharNo(evt,configuracoesDeCriacao){
    ...
    var noAdicionado =  cy.add(
      ...
      configuracoesDeCriacao.paridadeNoAnterior=0;
      noAdicionado.style({
        'background-color': '#00ff00','color':'white','font-size':'24px', 'border-width': 4,'border-color': 'white'
      }) ;
      configuracoesDeCriacao.contador++;
      
      adicionarListenersNos(noAdicionado,evt,configuracoesDeCriacao,cy);
